...
def get_all_versions():
    ...
    try:
        result = srun(
            [bot_cache["pkgs"][2], "-version"], capture_output=True, text=True
        )
...
async def get_stats(event, key="home"):
    ...
    if key == "home":
        ...
        official_v = (
            await cmd_exec(
                "curl -o latestversion.py https://gitlab.com/mysterysd.sd/WZML-X/-/raw/hk_wzmlx/bot/version.py -s && python3 latestversion.py && rm latestversion.py",
                True,
            )
        )[0]
        msg = BotTheme(
