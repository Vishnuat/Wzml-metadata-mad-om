...
async def get_media_info(path, metadata=False):
    ...
    if metadata:
        ...
        if (streams := ffresult.get("streams")) and streams[0].get(
            ...
            qual = f"{144 if qual <= 144 else 240 if qual <= 240 else 360 if qual <= 360 else 480 if qual <= 480 else 540 if qual <= 540 else 720 if qual <= 720 else 1080 if qual <= 1080 else 1440 if qual <= 1440 else 2160 if qual <= 2160 else 4320 if qual <= 4320 else 8640}p"
...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    lmetadata_key = user_dict.get("lmetadata_key", "")

    prefile_ = file_

    ###-----@HarixTGX-----######

    if lmetadata_key and dirpath and isMkv(file_):
        file_ = await change_metadata(file_, dirpath, lmetadata_key)
